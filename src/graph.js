...
export class Graph {
  ...
  getEdge(nodeA, nodeB) {
    return (
      this.edges
        .get(nodeA)
        .filter((edge) => Node.isEquivalent(edge.neighbor, nodeB))[0] || null
    );
  }
  ...
  #reconstructPath(cameFrom, current) {
    ...
    while (cameFrom.has(pathFromDestination[pathFromDestination.length - 1])) {
      pathFromDestination.push(
        cameFrom.get(pathFromDestination[pathFromDestination.length - 1])
      );
    }
    ...
    while (openSet.length > 0) {
      ...
      for (let neighbor of this.getNeighbors(currentGraphNode)) {
        ...
        if (!gScore.has(neighbor) || tentativeG < gScore.get(neighbor)) {
          ...
          fScore.set(neighbor, tentativeG + estimateCostFn(neighbor, endNode));
